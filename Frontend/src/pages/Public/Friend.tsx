...
import { UserStatusCxt } from "@/pages/App";

export default function Friends({id} : {id: number}){
    ...
    useEffect(()=> {
        ...
        const fetchAvatar = async (otherId: number) => {
            const res: undefined | string | Blob | MediaSource = await getUserAvatar(otherId);
            if (res !== "error")
                return res;
            else
        ...
        const fetchData = async ()=> {
            ...
            {            
                for (let i = 0; i < fetchedFriends.length; i++){
                    ...
                    let found = undefined;
                    if (usersStatus) {
                        found = usersStatus.find((x: IUserStatus) => x.key === fetchedFriends[i].id);
                        if (found)
                            row.userModel.status = found.userModel.status;
                    }
